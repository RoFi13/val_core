...
from PySide2.QtGui import QIcon, QPainter, QPaintEvent
...
MODULE_PATH = cpath.get_parent_directory(PurePath(__file__).as_posix())

# Resources filepath
# RSRC_PATH = os.path.join(MODULE_PATH, "resources").replace("\\", "/")
RSRC_PATH = PurePath(MODULE_PATH, "resources").as_posix()
...
def build_banner_button():
    ...
    # Customize button
    banner_image_path = PurePath(RSRC_PATH, "images/project_banner.png")
    LOG.debug("Banner image path: %s", banner_image_path)

    banner_icon = QIcon(banner_image_path.as_posix())
...
def convert_path_to_pyside_object(name):
    ...
    if ptr is not None:
        result = shiboken2.wrapInstance(int(ptr), QWidget)
